...
public class MsgHandler extends AbstractHandler {
	...
	@Autowired
	WeChatContentServer weChatContentServer;

  @Override
  public WxMpXmlOutMessage handle(WxMpXmlMessage wxMessage,
                                  Map<String, Object> context, WxMpService weixinService,
                                  WxSessionManager sessionManager) {
	this.logger.info("000000000000000000000000000000000000");
...
//    String content = "收到信息内容：" + JsonUtils.toJson(wxMessage);
    ...
    if (wxMessage.getMsgType().equals(XmlMsgType.TEXT)){
	    	WeChatPo findOne = weChatContentServer.matchContent(content);
	    	if (findOne != null){
	    		content = findOne.getContent();
	    	}
